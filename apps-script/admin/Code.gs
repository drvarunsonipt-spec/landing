...
function listBlockedSlots() {
  var sh = sheet_(TAB_BLOCKED, BLOCKED_COLS);
  var rows = sh.getDataRange().getValues();
  var out = [];
  for (var i = 1; i < rows.length; i++) {
    out.push({
...
function addBlockedSlot(date, time, reason) {
  if (!date) return { ok: false, error: 'date_required' };
  var sh = sheet_(TAB_BLOCKED, BLOCKED_COLS);
  sh.appendRow([date, time || '', reason || 'Blocked by admin']);
  return { ok: true };
}

function deleteBlockedSlot(row) {
  var sh = sheet_(TAB_BLOCKED, BLOCKED_COLS);
  sh.deleteRow(row);
  return { ok: true };
}
